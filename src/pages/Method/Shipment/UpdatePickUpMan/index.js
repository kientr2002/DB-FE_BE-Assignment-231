...
const handleSuccessClick = async () => {
  try {
    const response = await axios.post("http://localhost:8080/api/v1/shipments/update-pickup-man", formData, {
      headers: {
      ...
      <>
        ...
        <div className={styles.components}>
          <div className={styles.header} style={{padding: "2rem"}}>
            <h3 className={styles.title}>Update Pick-Up Man</h3>
          </div>
          <Grid
            container
            spacing={5}
            ...
            <Grid item>
              ...
              <CardContent style={{ userSelect: "none" }}>
                <Form.Control
                  size="lg"
                  type="text"
                  placeholder="pickupMan_SSN"
                  value={formData.pickupMan_SSN}
                  onChange={(e) =>
                    handleInputChange("pickupMan_SSN", e.target.value)
                  }
                />
              </CardContent>
              <CardContent style={{ userSelect: "none" }}>
...
export default UpdatePickUpMan;
