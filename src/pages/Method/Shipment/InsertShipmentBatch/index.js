...
const handleSuccessClick = async () => {
  try {
    const response = await axios.post("http://localhost:8080/api/v1/shipment-batchs/insert", formData, {
      headers: {
      ...
      <>
        ...
        <div className={styles.components}>
          <div className={styles.header} style={{padding: "2rem"}}>
            <h3 className={styles.title}>Insert Shipment Batch</h3>
          </div>
          <Grid
            container
            spacing={5}
            ...
            <Grid item>
              <CardContent style={{ userSelect: "none" }}>
                <Form.Control
                  size="lg"
                  type="text"
                  placeholder="transportation_id"
                  value={formData.transportation_id}
                  onChange={(e) =>
                    handleInputChange("transportation_id", e.target.value)
                  }
                />
              </CardContent>
              <CardContent style={{ userSelect: "none" }}>
...
export default InsertShipmentBatch;
