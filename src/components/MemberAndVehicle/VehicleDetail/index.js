...
export const VehicleDetail = ({ data }) => {
	...
	return (
		<div
			...
			<span style={{ fontWeight: "bold", fontSize: "0.9rem" }}>Lịch sử sử dụng: </span>
			...
			<span style={{ fontWeight: "bold", fontSize: "0.9rem" }}>Lịch sử sửa chữa, bảo dưỡng:</span>
			<Table size="small" style={{ margin: "0.5rem 0" }}>
				<TableHead>
					<TableRow>
						{["Ngày thực hiện", "Cơ sở thực hiện", "Nội dung", "Ghi chú"].map((cell) =>
							<TableCell align="center" style={{ fontWeight: "bold", fontSize: "0.8rem", border: "solid 1px black", backgroundColor: "#DFDFDF" }}>{cell}</TableCell>
						)}
					</TableRow>
				</TableHead>
				<TableBody>
					{data.repairHistory.map((item) =>
						<TableRow>
							{[item.date, item.gara, item.content, item.note].map((cell) =>
								<TableCell align="center" style={{ border: "solid 1px black", fontSize: "0.8rem" }}>{cell}</TableCell>
							)}
						</TableRow>
					)}
